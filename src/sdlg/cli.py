...
def cmd_run(args):
    cfg = _load_recipe(args.recipe) if args.recipe else {}
    ...
    gen = TextGenerator(
        ...
        chat=cfg.get("chat", args.chat),
        system_text=cfg.get("system", args.system),
        seed=cfg.get("seed", args.seed),
        deterministic=cfg.get("deterministic", args.deterministic),
    )
    n_per_prompt = cfg.get("n_per_prompt", args.n_per_prompt)
    lang = cfg.get("lang", args.lang)

    rows = gen.generate(prompts, n_per_prompt=n_per_prompt)
    pd.DataFrame(rows).to_csv(args.out, index=False, encoding="utf-8-sig")

    responses = [r["response"] for r in rows]
    rep = summarize_quality(responses, target_lang=lang)
    gates = pass_fail(rep)
    ...
    def main():
        ...
        r.add_argument("--recipe", help="JSONレシピファイル（指定すると各オプションを上書き）")
